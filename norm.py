...
if __name__ == "__main__":
    ...
    if "genWeight" not in weights:
        print("NOTE: '--weights' does not contain 'genWeight'. "
              "Normalization uses SumgenWeight from cutflow bin 1; "
              "for consistent MC normalization pass:  --weights 'genWeight * (other weights)'.")      
    ...
    if not args.signals_only:
        ...
        for category, sample_type in backgrounds.items():
            for proc_name, proc_info in sample_type.items():
                ...
                sumgenw_by_proc.setdefault(proc_name, 0.0)
                ...
                for path in proc_info["files"]:
                    ...
                    sum_genw_file = cutflow_hist.GetBinContent(1)
                    ngen_file     = cutflow_hist.GetBinContent(2)

                    sumgenw_by_proc[proc_name] += sum_genw_file
                    ngen_by_proc[proc_name]    += ngen_file
                    
                    total_events = ngen_file 
                    ...
                    print(f"Stored genWeighted hist {hist_name} with entries={h.GetEntries()} and integral={h.Integral(0, h.GetNbinsX()+1)} \n")
                    root_file.Close()
        ...
        for category, sample_type in backgrounds.items():
            for proc_name, proc_info in sample_type.items():
                ...
                if xsec_pb is None:
                    continue
                sum_genw = sumgenw_by_proc.get(proc_name, 0.0)
                ngen     = ngen_by_proc.get(proc_name, 0.0)
                

                w_proc = (xsec_pb * lumipb) / sum_genw
                print(f"[{proc_name}] weight = {w_proc}  (xsec={xsec_pb} pb, No. of events before Skimming={ngen}) \n")
...
##############################################################################
    ...
    sum_genw_1 = cutflow_1.GetBinContent(1)
    total_events_1 = cutflow_1.GetBinContent(2)
    sig_weight_1 = (sig_info["xsec"] * lumipb) / sum_genw_1
    ...
    sum_genw_2 = cutflow_2.GetBinContent(1)
    total_events_2 = cutflow_2.GetBinContent(2)
    sig_weight_2 = (sig_info["xsec"] * lumipb) / sum_genw_2
    ...
    sum_genw_3 = cutflow_3.GetBinContent(1) 
    total_events_3 = cutflow_3.GetBinContent(2)
    sig_weight_3 = (sig_info["xsec"] * lumipb) / sum_genw_3
    ...
    sum_genw_4 = cutflow_4.GetBinContent(1)
    total_events_4 = cutflow_4.GetBinContent(2)
    sig_weight_4 = (sig_info["xsec"] * lumipb) / sum_genw_4
    ...
    if not args.signals_only:
        ...
        frame = ROOT.TH1F("frame", hist_title, int(bin_values[0]), bin_values[1], bin_values[2])
    ...
    theLegend.Draw()
    canvas.Draw()
    mode = "signals_only" if args.signals_only else "signal_with_backgrounds"
    canvas.SaveAs(f"{args.year}_{args.Channel}_{variable}_{mode}.png")
    end = time.time()
